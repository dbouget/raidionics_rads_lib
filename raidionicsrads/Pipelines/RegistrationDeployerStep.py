...
class RegistrationDeployerStep(AbstractPipelineStep):
    ...
    def __apply_registration(self):
        ...
        if self._fixed_volume_uid == 'MNI':
            ...
            fixed_filepath = self._patient_parameters._radiological_volumes[self._fixed_volume_uid]._usable_input_filepath
        ...
        if self._moving_volume_uid == 'MNI':
            ...
            moving_filepath = self._patient_parameters._radiological_volumes[self._moving_volume_uid]._usable_input_filepath

        fp = self._registration_runner.apply_registration_transform(moving=moving_filepath, fixed=fixed_filepath,
                                                                    interpolation='bSpline')
        new_fp = os.path.join(self._patient_parameters._radiological_volumes[self._moving_volume_uid]._output_folder,
        ...
        self._patient_parameters._radiological_volumes[self._moving_volume_uid].include_registered_volume(filepath=new_fp,
                                                                                                          registration_uid=self._registration_instance._unique_id,
    ...
    def __apply_registration_annotations(self):
        ...
        for anno in self._patient_parameters.get_all_annotations_uids_radiological_volume(volume_uid=self._moving_volume_uid):
            annotation = self._patient_parameters._annotation_volumes[anno]
            moving_filepath = annotation._usable_input_filepath

            fp = self._registration_runner.apply_registration_transform(moving=moving_filepath, fixed=fixed_filepath,
                                                                        interpolation='nearestNeighbor')
            new_fp = os.path.join(self._patient_parameters._radiological_volumes[self._moving_volume_uid]._output_folder,
                                  self._fixed_volume_uid + '_space',
                                  self._moving_volume_uid + '_label_' + str(annotation._annotation_type) + '_registered_to_' + self._fixed_volume_uid + '.nii.gz')
            os.makedirs(os.path.dirname(new_fp), exist_ok=True)
            shutil.copyfile(fp, new_fp)
    ...
    def __apply_registration_atlas_space(self):
        # fixed_filepath = ResourcesConfiguration.getInstance().mni_atlas_filepath_T1
        fixed_filepath = self._patient_parameters._radiological_volumes[self._moving_volume_uid]._usable_input_filepath
        dump_folder = os.path.join(self._patient_parameters._radiological_volumes[self._moving_volume_uid]._output_folder,
                                   'Cortical-structures')
        ...
        # @TODO. Subcortical structures case to finish!
        ...
        #         dump_filename = os.path.join(self._registration_runner.registration_folder, 'Subcortical-structures',
        ...
        #
        #         self._registration_runner.apply_registration_inverse_transform(
