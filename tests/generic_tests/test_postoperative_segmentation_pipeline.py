...
def test_postoperative_segmentation_parallel_pipeline(test_dir, tmp_path):
    logging.basicConfig()
    logging.getLogger().setLevel(logging.DEBUG)
    logging.info("Running postoperative segmentation parallel pipeline unit test.\n")

    logging.info("Preparing configuration file.\n")
    try:
        output_folder = os.path.join(tmp_path, "results", "output_postop_seg_para_package")
        if os.path.exists(output_folder):
            shutil.rmtree(output_folder)
        os.makedirs(output_folder)

        test_raw_input_fn = os.path.join(test_dir, "patients", 'patient-UnitTest2')
        tmp_test_input_fn = os.path.join(tmp_path, "results", "inputs_postop_seg_para_package")
        if os.path.exists(tmp_test_input_fn):
            shutil.rmtree(tmp_test_input_fn)
        shutil.copytree(test_raw_input_fn, tmp_test_input_fn)
        ...
        rads_config.add_section('System')
        rads_config.set('System', 'gpu_id', "-1")
        rads_config.set('System', 'num_workers', "2")
        rads_config.set('System', 'input_folder', os.path.join(tmp_test_input_fn, "inputs"))
        rads_config.set('System', 'output_folder', output_folder)
        rads_config.set('System', 'model_folder', os.path.join(test_dir, "models"))
        rads_config.set('System', 'pipeline_filename', os.path.join(output_folder, 'test_pipeline.json'))
        ...
        logging.error(f"Error during segmentation parallel pipeline unit test with: {e}\n {traceback.format_exc()}.\n")
        raise ValueError("Error during segmentation parallel pipeline unit test with.\n")
    finally:
        if os.path.exists(tmp_test_input_fn):
            shutil.rmtree(tmp_test_input_fn)
        if os.path.exists(output_folder):
            shutil.rmtree(output_folder)
    logging.info("Postoperative segmentation parallel pipeline package test succeeded.\n")