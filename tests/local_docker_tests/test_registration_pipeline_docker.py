...
def test_registration_pipeline_docker(test_dir, tmp_path):
    ...
    output_folder = os.path.join(tmp_path, "output_reg_docker")
    if os.path.exists(output_folder):
        shutil.rmtree(output_folder)
    os.makedirs(output_folder)

    test_raw_input_fn = os.path.join(test_dir, "patients", 'patient-UnitTest1')
    tmp_test_input_fn = os.path.join(tmp_path, "results", "input_reg_docker")
    ...
    try:
        ...
        rads_config.add_section('System')
        rads_config.set('System', 'gpu_id', "-1")
        rads_config.set('System', 'input_folder', tmp_test_input_fn.replace(tmp_path, '/workspace/resources'))
        rads_config.set('System', 'output_folder', output_folder.replace(tmp_path, '/workspace/resources'))
        rads_config.set('System', 'model_folder', os.path.join(test_dir, "models").replace(test_dir, '/workspace/resources'))
        ...
        try:
            ...
            cmd_docker = ['docker', 'run', '-v', '{}:/workspace/resources'.format(test_dir),
                          '--network=host', '--ipc=host', '--user', str(os.geteuid()), 'dbouget/raidionics-rads:v1.3-py39-cpu',
                          '-c', '/workspace/resources/output_reg_docker/rads_config.ini', '-v', 'debug']
